...
_parent = [(_conversation # 0), _this] call ACS_fnc_system_select;
_children = _parent # 1;
...
if (_childAmount > 0) then {
	...
	lbSetCurSel [1500, -1]; {
		...
		if ((_condition == "") || {call compile _condition}) then {
			_array = _this;
			_topicName = (call compile ((_x # 0) # 0)) # 0; //first select gets the entire topic array, second select gets topic array, third select gets the name
